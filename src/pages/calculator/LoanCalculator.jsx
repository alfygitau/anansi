    ...
    <>
      <div className="min-h-screen bg-slate-50 text-primary pb-20">
        ...
        <div className="max-w-6xl sm:px-4 mx-auto">
          ...
          <section className="bg-white rounded-[10px] p-4 border border-slate-100 mb-10">
            {/* Section Header with Strategy Toggle */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <InputGroup
                ...
                value={rate}
                placeholder={isMonthly ? "3.0" : "14.0"}
                onChange={(e) => setRate(e.target.value)}
                suffix="%"
              />
              <InputGroup
                label="Loan Duration"
                icon={<Calendar size={20} className="text-sky-500" />}
                value={period}
...
const InputGroup = ({ label, icon, type, ...props }) => (
  <div className="space-y-2">
    ...
    {/* Input Container */}
    <div className="group flex items-center bg-white border border-slate-200 rounded-2xl overflow-hidden focus-within:ring-2 focus-within:ring-sky-500/20 focus-within:border-sky-500 transition-all">
      {/* Icon Prefix */}
      <div className="pl-4 pr-3 text-slate-400 group-focus-within:text-sky-500 transition-colors">
        {icon}
      </div>
      ...
      <input
        {...props}
        type={type}
        className="w-full bg-transparent border-none p-4 text-lg font-bold text-[#042159] placeholder:text-slate-300 outline-none focus:ring-0"
      />
    </div>
  </div>
);
